...
SELECT
    ...
    COALESCE(
        ...
        FLOOR(UNIFORM(0::FLOAT,1::FLOAT,RANDOM()) * 5)::INT  -- Simulated if no outage tracking
    ) AS ACTIVE_OUTAGES,
    ...
    COALESCE(
        ...
        ROUND(150 + UNIFORM(0::FLOAT,1::FLOAT,RANDOM()) * 50, 2)  -- Simulated ~150-200 MW
    ) AS TOTAL_LOAD_MW,
    
    -- Crew metrics (simulated - would come from workforce management)
    FLOOR(5 + UNIFORM(0::FLOAT,1::FLOAT,RANDOM()) * 10)::INT AS CREWS_ACTIVE,
    
    -- Restoration time (simulated - would come from outage history)
    ROUND(30 + UNIFORM(0::FLOAT,1::FLOAT,RANDOM()) * 60, 1) AS AVG_RESTORATION_MINUTES;
...
SELECT 
    ...
    COALESCE(
        ...
        ROUND(40 + UNIFORM(0::FLOAT,1::FLOAT,RANDOM()) * 40, 2)  -- Simulated 40-80% if no data
    ) AS AVG_LOAD_PCT,
...
SELECT 
    ...
    COALESCE(
        ...
        CASE c.VOLTAGE_CLASS 
            ...
            ELSE 12470 
        END * (0.98 + UNIFORM(0::FLOAT,1::FLOAT,RANDOM()) * 0.04)  -- 98-102% of nominal
    ) AS AVG_VOLTAGE,
    -- Outage indicator
...
SELECT 
    ...
    ROUND(99.5 + UNIFORM(0::FLOAT,1::FLOAT,RANDOM()) * 0.4, 2) AS RELIABILITY_PCT,
    ROUND(0.5 + UNIFORM(0::FLOAT,1::FLOAT,RANDOM()) * 1.5, 2) AS SAIDI_MINUTES
FROM <% database %>.PRODUCTION.SUBSTATIONS s;
...
SELECT
    ...
    -95.7 + UNIFORM(0::FLOAT,1::FLOAT,RANDOM()) * 0.7 AS LONGITUDE,
    29.7 + UNIFORM(0::FLOAT,1::FLOAT,RANDOM()) * 0.3 AS LATITUDE,
    ROUND(8 + UNIFORM(0::FLOAT,1::FLOAT,RANDOM()) * 25, 1) AS HEIGHT_M,
    ROUND(2 + UNIFORM(0::FLOAT,1::FLOAT,RANDOM()) * 6, 1) AS CANOPY_RADIUS_M,
    -- Risk scoring
    ...
    CASE 
        ...
        ELSE 'safe'
    END AS RISK_LEVEL,
    ROUND(2 + UNIFORM(0::FLOAT,1::FLOAT,RANDOM()) * 50, 1) AS DISTANCE_TO_LINE_M,
    'LINE_' || FLOOR(UNIFORM(0::FLOAT,1::FLOAT,RANDOM()) * 1000)::VARCHAR AS NEAREST_LINE_ID,
    CASE MOD(SEQ4(), 3) WHEN 0 THEN 'transmission' WHEN 1 THEN 'distribution' ELSE 'service' END AS NEAREST_LINE_CLASS,
    ROUND(10 + UNIFORM(0::FLOAT,1::FLOAT,RANDOM()) * 20, 1) AS FALL_ZONE_M,
    'Tree within ' || ROUND(2 + UNIFORM(0::FLOAT,1::FLOAT,RANDOM()) * 50, 0)::VARCHAR || 'm of power line' AS RISK_EXPLANATION,
    CASE MOD(SEQ4(), 4) WHEN 0 THEN 'pole' WHEN 1 THEN 'transformer' WHEN 2 THEN 'line' ELSE 'substation' END AS NEAREST_ASSET_TYPE,
    ROUND(5 + UNIFORM(0::FLOAT,1::FLOAT,RANDOM()) * 100, 1) AS DISTANCE_TO_ASSET_M,
...
SELECT 
    ...
    CASE COALESCE(c.STATUS, 'ENERGIZED')
        WHEN 'OUTAGE' THEN DATEADD('minute', -FLOOR(UNIFORM(0::FLOAT,1::FLOAT,RANDOM()) * 120)::INT, CURRENT_TIMESTAMP())
        ELSE NULL
    END AS OUTAGE_START_TIME,
    CASE COALESCE(c.STATUS, 'ENERGIZED')
        WHEN 'OUTAGE' THEN FLOOR(UNIFORM(0::FLOAT,1::FLOAT,RANDOM()) * 500)::INT
        ELSE 0
    END AS CUSTOMERS_AFFECTED,
    CASE COALESCE(c.STATUS, 'ENERGIZED')
        WHEN 'OUTAGE' THEN 
            CASE MOD(FLOOR(UNIFORM(0::FLOAT,1::FLOAT,RANDOM()) * 5)::INT, 5)
...
SELECT 
    ...
    s.SUBSTATION_ID,
    0.8 + UNIFORM(0::FLOAT,1::FLOAT,RANDOM()) * 0.2 AS DEGREE_CENTRALITY,
    0.3 + UNIFORM(0::FLOAT,1::FLOAT,RANDOM()) * 0.5 AS BETWEENNESS_CENTRALITY,
    0.4 + UNIFORM(0::FLOAT,1::FLOAT,RANDOM()) * 0.4 AS LOAD_FACTOR,
    5 + UNIFORM(0::FLOAT,1::FLOAT,RANDOM()) * 30 AS AGE_YEARS,
    60 + UNIFORM(0::FLOAT,1::FLOAT,RANDOM()) * 35 AS HEALTH_SCORE,
    0.5 + UNIFORM(0::FLOAT,1::FLOAT,RANDOM()) * 0.4 AS CRITICALITY_SCORE,
...
SELECT 
    ...
    0.3 + UNIFORM(0::FLOAT,1::FLOAT,RANDOM()) * 0.4 AS DEGREE_CENTRALITY,
    0.1 + UNIFORM(0::FLOAT,1::FLOAT,RANDOM()) * 0.3 AS BETWEENNESS_CENTRALITY,
    0.3 + UNIFORM(0::FLOAT,1::FLOAT,RANDOM()) * 0.5 AS LOAD_FACTOR,
    AGE_YEARS,
    HEALTH_SCORE,
    0.2 + UNIFORM(0::FLOAT,1::FLOAT,RANDOM()) * 0.5 AS CRITICALITY_SCORE,
...
SELECT 
    ...
    100 + UNIFORM(0::FLOAT,1::FLOAT,RANDOM()) * 500 AS LENGTH_METERS,
    ROUND((100 + UNIFORM(0::FLOAT,1::FLOAT,RANDOM()) * 500) / 1000, 4) AS DISTANCE_KM,
    0.1 + UNIFORM(0::FLOAT,1::FLOAT,RANDOM()) * 0.5 AS IMPEDANCE_OHMS,
    ROUND(0.01 + UNIFORM(0::FLOAT,1::FLOAT,RANDOM()) * 0.05, 4) AS IMPEDANCE_PU,
    n.VOLTAGE_LEVEL AS VOLTAGE_LEVEL,
    200 + UNIFORM(0::FLOAT,1::FLOAT,RANDOM()) * 300 AS CAPACITY_AMPS,
    0.3 + UNIFORM(0::FLOAT,1::FLOAT,RANDOM()) * 0.5 AS CURRENT_FLOW_PCT
FROM GRID_NODES n
...
CREATE TABLE IF NOT EXISTS OUTAGE_RESTORATION_TRACKER (
    ...
    SUBSTATION_ID VARCHAR(50),
    -- 2026-07-29: TRANSFORMER_ID is REQUIRED, not optional.
    -- 08_semantic_view.sql declares
    --   outages(TRANSFORMER_ID) REFERENCES xfmr(TRANSFORMER_ID)
    -- so a fresh deploy failed with "invalid identifier 'TRANSFORMER_ID'". The
    -- comment in script 08 claims TRANSFORMER_ID is "excluded (accessible via xfmr
    -- join)", but its own RELATIONSHIPS block joins ON it -- the comment and the
    -- code disagreed, and the code is what runs. AFFECTED_TRANSFORMERS below is a
    -- COUNT, not a key, so it cannot serve the relationship.
...
USING (
    SELECT 
        ...
        'CKT_' || FLOOR(UNIFORM(0::FLOAT,1::FLOAT,RANDOM()) * 500)::VARCHAR AS CIRCUIT_ID,
        'SUB_' || FLOOR(UNIFORM(0::FLOAT,1::FLOAT,RANDOM()) * 50)::VARCHAR AS SUBSTATION_ID,
        DATEADD('hour', -FLOOR(UNIFORM(0::FLOAT,1::FLOAT,RANDOM()) * 24)::INT, CURRENT_TIMESTAMP()) AS OUTAGE_START_TIME,
        CASE MOD(SEQ4(), 5)
            WHEN 0 THEN 'ACTIVE'
        ...
        CASE MOD(SEQ4(), 5)
            ...
            ELSE 'UNKNOWN'
        END AS CAUSE,
        FLOOR(50 + UNIFORM(0::FLOAT,1::FLOAT,RANDOM()) * 500)::INT AS AFFECTED_CUSTOMERS,
        FLOOR(1 + UNIFORM(0::FLOAT,1::FLOAT,RANDOM()) * 10)::INT AS AFFECTED_TRANSFORMERS,
        'CREW_' || FLOOR(UNIFORM(0::FLOAT,1::FLOAT,RANDOM()) * 20)::VARCHAR AS CREW_ASSIGNED,
        DATEADD('hour', FLOOR(UNIFORM(0::FLOAT,1::FLOAT,RANDOM()) * 4)::INT, CURRENT_TIMESTAMP()) AS ESTIMATED_RESTORATION
