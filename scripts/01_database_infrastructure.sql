...
SET admin_role = COALESCE($admin_role, 'FLUX_ADMIN_ROLE');
SET user_role = COALESCE($user_role, 'FLUX_USER_ROLE');
...
-- User gets usage on key schemas (use dynamic SQL for qualified names)
EXECUTE IMMEDIATE 'GRANT USAGE ON SCHEMA ' || $database || '.PRODUCTION TO ROLE IDENTIFIER(''' || $user_role || ''')';
EXECUTE IMMEDIATE 'GRANT USAGE ON SCHEMA ' || $database || '.APPLICATIONS TO ROLE IDENTIFIER(''' || $user_role || ''')';
EXECUTE IMMEDIATE 'GRANT USAGE ON SCHEMA ' || $database || '.ML TO ROLE IDENTIFIER(''' || $user_role || ''')';
...
EXECUTE IMMEDIATE 'GRANT SELECT ON FUTURE TABLES IN SCHEMA ' || $database || '.PRODUCTION TO ROLE IDENTIFIER(''' || $user_role || ''')';
EXECUTE IMMEDIATE 'GRANT SELECT ON FUTURE TABLES IN SCHEMA ' || $database || '.APPLICATIONS TO ROLE IDENTIFIER(''' || $user_role || ''')';
EXECUTE IMMEDIATE 'GRANT SELECT ON FUTURE TABLES IN SCHEMA ' || $database || '.ML TO ROLE IDENTIFIER(''' || $user_role || ''')';

-- Views: SELECT for read access
EXECUTE IMMEDIATE 'GRANT SELECT ON FUTURE VIEWS IN SCHEMA ' || $database || '.PRODUCTION TO ROLE IDENTIFIER(''' || $user_role || ''')';
EXECUTE IMMEDIATE 'GRANT SELECT ON FUTURE VIEWS IN SCHEMA ' || $database || '.APPLICATIONS TO ROLE IDENTIFIER(''' || $user_role || ''')';

-- Semantic Views: SELECT (critical for Cortex Analyst)
EXECUTE IMMEDIATE 'GRANT SELECT ON FUTURE SEMANTIC VIEWS IN SCHEMA ' || $database || '.APPLICATIONS TO ROLE IDENTIFIER(''' || $user_role || ''')';

-- Functions, Procedures, Stages: USAGE
EXECUTE IMMEDIATE 'GRANT USAGE ON FUTURE FUNCTIONS IN SCHEMA ' || $database || '.APPLICATIONS TO ROLE IDENTIFIER(''' || $user_role || ''')';
EXECUTE IMMEDIATE 'GRANT USAGE ON FUTURE PROCEDURES IN SCHEMA ' || $database || '.APPLICATIONS TO ROLE IDENTIFIER(''' || $user_role || ''')';
EXECUTE IMMEDIATE 'GRANT USAGE ON FUTURE STAGES IN SCHEMA ' || $database || '.APPLICATIONS TO ROLE IDENTIFIER(''' || $user_role || ''')';

-- Cortex Search Services: USAGE (for RAG and search)
EXECUTE IMMEDIATE 'GRANT USAGE ON FUTURE CORTEX SEARCH SERVICES IN SCHEMA ' || $database || '.APPLICATIONS TO ROLE IDENTIFIER(''' || $user_role || ''')';
...
EXECUTE IMMEDIATE 'CREATE STAGE IF NOT EXISTS ' || $database || '.RAW.FLUX_SEED_DATA DIRECTORY = (ENABLE = TRUE) COMMENT = ''Internal stage for seed data files (parquet, CSV)''';
EXECUTE IMMEDIATE 'GRANT READ ON STAGE ' || $database || '.RAW.FLUX_SEED_DATA TO ROLE IDENTIFIER(''' || $user_role || ''')';
