...
#   sql_execute "ALTER WAREHOUSE FLUX_WH SUSPEND"   -- hard kill
#
# Stdout event format (one line per event, line-buffered):
#   [HH:MM:SS] ORCH start   run_id=…  total_days=N  meter_sample=N  start_date=…  dry_run=…
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        *)
            ...
            echo "Usage: $0 --connection NAME --run-id ID --start-date YYYY-MM-DD [--total-days N] [--meter-sample N] [--dry-run]" >&2
            exit 1
            ;;
    esac
done
...
control_status() {
    snow sql \
        -q "SELECT CONTROL_VALUE FROM FLUX_DB.PRODUCTION.AMI_GENERATION_CONTROL WHERE CONTROL_KEY='STATUS'" \
        --format=plain \
...
emit "ORCH start  run_id=$RUN_ID  total_days=$TOTAL_DAYS  meter_sample=$METER_SAMPLE  start_date=$START_DATE  dry_run=$DRY_RUN"
...
for ((i=1; i<=TOTAL_DAYS; i++)); do
    ...
    CALL_RESULT=$(snow sql \
        -q "CALL FLUX_DB.PRODUCTION.GENERATE_AMI_CHUNK(
                '$RUN_ID',
                $i,
    ...
    VAL_RESULT=$(snow sql \
        -q "CALL FLUX_DB.PRODUCTION.VALIDATE_AMI_CHUNK('$RUN_ID', $i)" \
        --format=json \
        --connection "$CONNECTION" 2>&1)
    VAL_RC=$?
    ...
    if [[ $VAL_RC -ne 0 ]]; then
        ...
        snow sql \
            -q "UPDATE FLUX_DB.PRODUCTION.AMI_GENERATION_CONTROL SET CONTROL_VALUE='PAUSED', UPDATED_AT=CURRENT_TIMESTAMP() WHERE CONTROL_KEY='STATUS'" \
