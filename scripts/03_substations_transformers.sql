...
CREATE OR ALTER TABLE TRANSFORMER_METADATA (
    -- Primary key
    TRANSFORMER_ID VARCHAR(50) NOT NULL PRIMARY KEY,
    ...
    PRIMARY_VOLTAGE_KV NUMBER(5,1),
    PHASE_CODE VARCHAR(10),  -- ABC, AB, A, B, C
    ...
    -- Load metrics
    CURRENT_LOAD_KVA NUMBER(10,1),
    PEAK_LOAD_KVA NUMBER(10,1),
    LOAD_UTILIZATION_PCT NUMBER(5,2),
    METER_COUNT NUMBER(10,0),
    
    -- CIM-compliant fields (Added Jan 2026)
    TRANSFORMER_ROLE VARCHAR(20),  -- SERVICE, SPLIT, NETWORK
    PARENT_TRANSFORMER_ID VARCHAR(50),
    
    -- Computed columns for semantic view compatibility
    AGE_YEARS NUMBER(10,0) AS (YEAR(CURRENT_DATE()) - INSTALL_YEAR),
    LOCATION_AREA VARCHAR(50) AS (
        CASE 
    ...
    FEEDER_ID VARCHAR(30) AS ('FEEDER-' || COALESCE(RIGHT(CIRCUIT_ID, 3), '000')),
    ASSET_ID VARCHAR(50) AS (TRANSFORMER_ID),
    LOAD_SERVING_CLASS VARCHAR(20) AS (
        CASE
